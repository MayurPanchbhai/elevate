...
export const WomenDetails = () => {
    const { theme, WTshirt , handleLiked ,handleCarItem } = useContext(DataContext);
    const paar = useParams();

    const Specs = WTshirt.filter((curr) => curr.id == paar.id);
    const { name, ImSrc,price, images , liked , id , type ,ActualPrice , color ,brand ,description ,sizes , rating ,root , quantity} = Specs[0]; 
