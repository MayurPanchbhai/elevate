...
import HeroCardImg from './Component/HeroCardImg';

const Men = () => {
  const {MTshirt,theme,handleCarItem , HeroCardImgMen ,MTShirtImg } = useContext(DataContext);
    ...
    <div className={`${theme ? "bg-black text-white" : "bg-white text-black"}  pt-32 w-full`}>
      ...
      <div className="hero-section w-full flex flex-col-reverse md:flex-row md:justify-evenly items-center gap-3 flex-wrap">
        <div className='hero-section-left w-full md:w-5/12 flex-col flex-wrap px-6 md:px-0 '>
          <h1 className='uppercase font-bold font-serif text-5xl md:text-8xl text-center md:text-left'>BE</h1>
          <h1 className='uppercase font-bold font-serif text-5xl md:text-8xl text-center md:text-left'>the</h1>
          <div className='flex gap-3 items-center flex-col  md:flex-row'>
            <HeroCardImg animatedImg={HeroCardImgMen}   />
            <h1 className='uppercase font-bold font-serif text-5xl md:text-8xl flex md:items-baseline gap-2 text-center md:flex-row flex-col '>man</h1>
            
          </div>

          <div className='flex justify-center md:justify-start'>
            <button type="button" className={`border-[1px] mt-3 flex border-black px-5 py-2 md:py-1 rounded-lg  font-bold font-serif capitalize items-center gap-3  md:hover:ml-8 transition-all duration-1000 md:hover:bg-blue-800  md:hover:border-blue-800 md:hover:text-white ${theme ? "border-white" : ""}  `}>
